...
def train_one_model(model_class, csv_paths, model_name):
    ...
    optimizer = optim.Adam(model.parameters(), lr=LEARNING_RATE, weight_decay=1e-5) # 增加L2正则化防止过拟合
    ...
    for epoch in range(EPOCHS):
        ...
        # 打印日志
        print(f"Epoch {epoch+1:02d}/{EPOCHS} | Train Loss: {avg_train_loss:.4f} | Val Loss: {avg_val_loss:.4f}", end="")
        ...
        if early_stop_counter >= PATIENCE:
            ...
            print(f"Training stopped. Best Validation Loss was: {best_val_loss:.4f}")
            break

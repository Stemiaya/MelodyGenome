...
class FitnessEvaluator:
    def __init__(self, model_type='combined', checkpoint_dir='./checkpoints'):
        """
        初始化评估器。建议在遗传算法开始前初始化一次，不要在循环里反复初始化。
        ...
        print(f"[FitnessEvaluator] Initializing {model_type} model on {self.device}...")
        ...
        if not os.path.exists(weight_path):
            raise FileNotFoundError(f"Model weights not found at {weight_path}. Did you train it?")
            
        # map_location确保在没有GPU的机器上也能加载GPU训练的模型
        self.model.load_state_dict(torch.load(weight_path, map_location=self.device))
        self.model.to(self.device)
        self.model.eval() # 开启评估模式 (这一点很重要，会关闭Dropout)
        
        # 3. 加载Key的编码器 (.pkl)
        # 因为模型只认识数字(0,1,2...)，不认识'C', 'F#'，所以需要加载训练时的编码器
        ...
        if not os.path.exists(encoder_path):
             raise FileNotFoundError(f"Key encoder not found at {encoder_path}.")
    ...
    def get_fitness(self, melody_list, key_str):
        ...
        melody_tensor = torch.LongTensor(melody_list).unsqueeze(0).to(self.device)
        ...
        key_tensor = torch.LongTensor([key_int]).unsqueeze(0).to(self.device) # -> [1, 1]
